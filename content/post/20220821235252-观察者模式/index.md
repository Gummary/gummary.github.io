...
系统中某些类应该只能有一个实例，例如驱动、线程池等资源类。单例模式就为了确保这些类在只被初始化一次同时提供全局访问。

单例模式需要将默认构造函数私有化，确保该类不会被其他类初始化，获取实例则通过静态成员函数`getInstance`完成。

多线程模式下单例模式需要加锁，必要时需要双重检查锁(详细可参考[Java中的双重检查锁](https://www.cnblogs.com/xz816111/p/8470048.html))。

单例模式与全局变量的一些区别：

1. 单例模式能够确保只有一个实例且提供全局访问，全局变量只能保证全局访问，不能保证唯一
2. 单例模式可以在类被用到时初始化。
...
状态模式和策略模式非常相似，但是二者之间的本质区别在于对象状态（具体策略）的过滤逻辑。状态模式的状态变化是客户端调用具体行为之后，自发改变的，客户端是无感知的；而策略模式是客户端主动选择不同的策略执行。