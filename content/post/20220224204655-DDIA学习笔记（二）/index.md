...
分区是将数据存储到不同的独立的节点上，这些节点之间不会共享资源。通过数据的划分提高访问数据的吞吐率。

partitioning和replication可以是两个独立的机制，一个分区可以有多个副本存储在其他节点上，这样可以既保证稳定性又保证伸缩性。

## Partitioning of Key-Value Data

分区是提高吞吐率，也即如果有10个分区，那么期望整个系统承载能力就是之前的10倍。因此需要数据尽可能均匀的分配到各个节点上。最简单的方式是随机分配，但是使用随机分配的问题在于，我们不知道具体的key会存储在哪个节点上，访问时需要访问每个节点，找到该值。

所以我们需要一种策略，既能够均匀的将数据分配到不同的节点上，在访问数据时又能根据key值在找到数据。

### Partitioning by Key Range

根据key的属性将key划分成不同的区间，例如如果key是英文的，那么可以根据字母的范围对key进行划分。 A-B存储一个节点，C-D存储一个节点等等。至于具体如何划分通常由管理员或系统自动划分。划分的依据就是根据数据的特点让数据尽可能分散。

在每个分区内，可以对key进行排序，例如使用SSTable来存储数据。在查询时可以快速查找到对应的key，也可以快速的根据key进行范围查找。

使用区间进行划分的难点就在于如何均匀的划分区间。

### Partitioning by Hash of Key

如果key不能均匀的划分区间，那么我们可以对key进行映射，映射到一个均匀分布的值上。例如使用Hash算法，计算每个Key的hash值，然后根据key的hash值对key进行均匀划分。

使用hash值进行划分的缺点在于，无法对key进行范围查找，因为两个近似的key其hash值可能不同。一种折中的方式是采用数据的两个属性的串联作为key，用第一个数据计算hash值，找到对应的分区，在每个分区内部使用第二个属性进行排序。

### Skewed Workloads and Relieving Hot Spots

即使是采用了hash将key打散，仍会出现热点key，导致某个分区的请求量比其他节点更多。例如社网络中，某个大V主页的访问量一定比普通用户主页的访问量大。

对于当前的数据系统来说还没有有效的手段处理这种key，通常由应用程序进行处理。例如写入时给这个key后加随机的数字或字母，让这个key的写入请求分散到不同的节点上。但是在读取时，就需要从不同的节点上读取数据并进行组合。

## Partitioning and Secondary Indexes

如果数据使用了二级索引，情况会变得更加复杂。对于二级索引，有两种常见的解决方案：local index和global index。

其中Local index是每个分区只存储当前数据的二级索引。客户端写入数据时只需要修改一个节点上的索引及数据。但是当需要根据二级索引查询时，需要到每个节点上都查询该索引下的数据。

Global index是将一个索引都存储到一个节点上。为了防止该节点成为性能瓶颈，通常会将该索引下的值打散，每个值存储到一个节点上，类似于将索引的值划分区间，存储到不同的节点上。使用Global Index的好处在于，根据二级索引查找时，只需要访问一个分区即可。但是写入时，因为一个数据可能包含多个索引值，因此需要一次写入会涉及多个分区。

## Rebalancing Partitions

Rebalancing是当数据库无法满足当前存储的要求，需要扩容时进行的操作。Rebalancing通常有三个基本的要求：

1. Rebalance之后的请求的分配会更加均衡
2. Rebalance的过程不会影响数据的读写
3. Rebalance要尽可能少的移动数据。

在根据Key的Hash值进行分区时，我们是按照hash的范围进行分区的。而不是根据hash的mod来决定。原因是当增加新的分区时，每个key的mod都会发生变化，需要移动大量的数据。

一种常见的Rebalance方式是固定分区的数量。当有新的节点加入时，将旧节点上的一些分区分配到新的节点上，这样旧节点的流量就会被分流到新结点上，缓解压力。

这种模式下，分区数量在系统被建立时就已经确定了。因此选好分区的数量至关重要。如果分区数量过小，数据量很大，会导致每个节点承载的流量很大；如果分区数量过大，rebalance和failover的过程又会带来更多的成本。

另一种方式是让数据库自行决定分区数量。也即当存储的数量超过一定的阈值时，系统会自动将分区分裂，将其中一个分区的数据拷贝到另一个节点上。这种动态分区的方式的好处是分区的数量是与数据量呈正相关。

还有一种方式是让分区的数量与节点数量正相关，也即每个节点上分区数量是一定的，每当新增加一个节点，就会从其他节点上分裂一些分区放到新结点上。

## Request Routing

分区之后还有一个问题是，客户端如何确定该向哪个节点发起请求。一般来说有三种方案：

1. 客户端向任意一个节点发起请求，如果key在该节点上，则直接处理。否则由该节点向存有该key的节点发起请求，然后将返回值返回给客户端。
2. 使用一个代理节点，客户端每次都向该代理节点发起请求，代理节点保存了每个key的映射关系，由代理节点请求实际数据。
3. 客户端自身保存key的映射关系。

那么在这三种情况下，当发生rebalancing时，如何通知这些这些节点呢。
