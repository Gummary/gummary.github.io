...
比赛结束后大佬们都分享下了自己的方案，这里做个学习记录。我仔细看了下[第二名](https://tianchi.aliyun.com/forum/postDetail?spm=5176.12586969.1002.6.569861a2UigI8m&postId=325133)和[第三名](https://tianchi.aliyun.com/forum/postDetail?spm=5176.12586969.1002.3.569861a2UigI8m&postId=326699)的方案。

## 第二名方案

在**超时机制**上，使用的方式是直接使用上次成功请求的rtt+常量作为本次rtt，快速忽略异常超时调用。这种算法实现简单快速，而且能快速忽略最近的异常值。

然后该方案中的核心是对“吞吐量-并发度-延迟”的分析情况，这里引用他们的图如下：

{{< tfigure src="images/2022-02-09-16-31-29.png" title="吞吐量，延时与并发度关系" width="" class="align-center">}}

如果要做**限流**，就需要估计出最佳的并发大小，将并发度限制在最佳的位置处。该方案采用的估计算法是在预热阶段使用爬山算法，评测阶段动态微调：

> 第一阶段：基于爬山算法的最优容量探测： 固定并发度限制，统计一段时间间隔内的正确请求数量，根据本次和上一次不同并发度限制所统计的正确请求数，动态的调整并发度限制。假设步长设置的合理，在预热阶段结束后，并发度限制设置会大致收敛于蓝线左右。
> 第二阶段：基于延时反馈的最佳容量微调： 在消费端比较固定请求数的平均处理时间与历史总请求的平均处理时间，依此对服务端并发度限制进行微小的动态调整。

这么做的原因在于，统计TPS比较耗时，所以在评测时用延迟时间动态微调。

在该方案的**负载均衡**算法中，同样使用了最优并发度这个参数。该方案计算权重的方式如下：
...
在**超时机制**上，该方案使用的是平均值+随机系数。

该方案同样分析了"吞吐量-并发度-延迟"的关系，除了最佳并发量外，还计算了最佳的TPS。

**限流算法**中，直接只用了计算出的最佳并发量。在**负载均衡算**法上，使用了加权负载均衡算法，权重为最佳TPS。

除了上述的三个算法外，该方案还提出了一种整流算法。当当前的并发数大于最大并发值时，将请求加入到一个队列中；当并发数小于最大并发值时，从队列中取出请求。通过队列实现削峰填谷的作用，效果如下：

{{< tfigure src="images/2022-02-09-16-58-15.png" title="整流前" width="" class="align-center">}}

{{< tfigure src="images/2022-02-09-16-58-23.png" title="整流后" width="" class="align-center">}}
